    ...
    if(parent.getCurrentPropertyTarget().has_value())
    {
      if(parent.getCurrentPropertyTarget())
      {
        ...
        if (sceneDefOptional)
        {
          ...
          camChange |= ImGui::RadioButton("Free Camera", &cameraTarget, 0);
          camChange |= ImGui::RadioButton("Entity Camera", &cameraTarget, 1);
          if (camChange) cam.setUseEntity(cameraTarget == 0 ? false : true);
          ...
          if (camEntityOpt)
          {
            ...
            float fov;
            fov = cam.getFieldOfViewDegrees();

            if (ImGui::DragFloat("FOV", &fov, 0.1f, 1.f,180.f))
            {
              cam.setFieldOfViewDegrees(fov);
            }
          ...
          // Setup ShadowPassShader
          {
            ...
            if (ImGuiWidget::StringCombo("Shadow Pass", &shadowShaderIndex, shaderList))
            {
          ...
          // Setup FontShader
          {
            ...
            if (ImGuiWidget::StringCombo("Font", &fontShaderIndex, shaderList))
            {
          ...
          // Setup Sprite shader
          {
            ...
            if (ImGuiWidget::StringCombo("Sprite", &spriteShaderIndex, shaderList))
            {
          ...
          // Setup Environment Texture
          {
            ...
            if (ImGuiWidget::StringCombo("Texture", &envMapTexIndex, textureList))
            {
          ...
          // Environment Shader
          {
            ...
            if (ImGuiWidget::StringCombo("Shader", &envShaderIndex, shaderList))
            {
          ...
          if (ImGuiWidget::StringCombo("Input Script",&inputScriptIndex,scriptAssets))
          {
      ...
      if(parent.getCurrentPropertyTarget())
      {
        ...
        if (pDefOpt && sDefOpt)
        {
          ...
          if(ImGui::BeginPopupModal("Confirm Delete Scene"))
          {
            ...
            {
              dtCtx.getProjectBrowser().removeNodeSelection(sDef);
