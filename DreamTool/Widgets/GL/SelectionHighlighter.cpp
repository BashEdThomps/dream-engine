...
namespace octronic::dream::tool
{
  ...
  (EntityRuntime& selected)
  {
    ...
    LOG_INFO("SelectionHighlighter: SelectedEntity changed to {}",getSelectedEntityRuntime().getNameAndUuidString());
    updateGeometry();
  }
  ...
  ()
  {
    ...
    if (!mSelectedEntityRuntime.has_value())
    {
      return;
    }

    BoundingBox bounds = getSelectedEntityRuntime().getBoundingBox();
    vec3 min, max;
  ...
  void SelectionHighlighter::draw()
  {
    ...
    if (mSelectedEntityRuntime.has_value())
    {
      model = getSelectedEntityRuntime().getTransform().getMatrix();
    }
    else
    {
  ...
  EntityRuntime&
  SelectionHighlighter::getSelectedEntityRuntime()
  {
    return mSelectedEntityRuntime.value();
  }
}
