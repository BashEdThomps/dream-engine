...
// IBL
uniform samplerCube uIrradianceTexture;
...
{		
    ...
    float metallic = texture(uMetallicTexture, TexCoords).r;
    float roughness = texture(uRoughnessTexture, TexCoords).r;
    float ao = texture(uAOTexture, TexCoords).r;
    ...
    vec3 irradiance = texture(uIrradianceTexture, N).rgb;
    vec3 diffuse      = irradiance * albedo;
    
    // sample both the pre-filter map and the BRDF lut and combine them together as per the Split-Sum approximation to get the IBL specular part.
    const float MAX_REFLECTION_LOD = 4.0;
    vec3 prefilteredColor = textureLod(uPreFilterTexture, R,  roughness * MAX_REFLECTION_LOD).rgb;    
