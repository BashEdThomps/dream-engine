...
namespace octronic::dream
{
    ...
    File* StorageManager::openFile(const string& file_path)
    {
        ...
        auto file_itr = std::find_if(mOpenFiles.begin(), mOpenFiles.end(), [&](File* next_file){return next_file->getPath().compare(file_path) == 0;});
    ...
    void StorageManager::closeFile(File *file)
    {
        ...
        auto file_itr = std::find_if(mOpenFiles.begin(), mOpenFiles.end(), [&](File* next_file) { return next_file->getPath().compare(file->getPath()) == 0; });
    ...
    void StorageManager::closeDirectory(Directory* d)
    {
        ...
        auto dir_itr = std::find_if( mOpenDirectories.begin(), mOpenDirectories.end(),
                    [&](Directory* next_dir) { return next_dir->getPath().compare(d->getPath()) == 0; });
