...
namespace octronic::dream
{
  ...
  (ProjectRuntime& pr, ModelMesh& mesh)
  ...
  ()
  {
    LOG_TRACE("ModelInitMeshTask: for mesh: {}, Executing on Graphics thread",getMesh().getName());
    if (getMesh().loadIntoGL())
    {
  ...
  ModelMesh&
  ModelInitMeshTask::getMesh() const
  {
    return mMesh.get();
  }
